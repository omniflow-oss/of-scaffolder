...
const ensureService6a = async ({ svcDir }) => {
  if (!(await fs.pathExists(path.join(svcDir, "pom.xml")))) throw new Error(`Service not found: ${svcDir}`);
  return "OK";
};

const registerGolden6aGenerators = ({ plop, ctx, validators, utils }) => {
  plop.setGenerator("feature6a", {
    description: "Rev6A: add a feature module container under module/<module>/ (package regrouping only)",
    prompts: [
    ...
    actions: function (answers) {
      ...
      return [
        async () => ensureService6a({ svcDir }),
        { type: "add", path: path.join(moduleDir, ".gitkeep"), templateFile: ctx.template("service", "gitkeep.hbs") }
      ];
    }
  });

  plop.setGenerator("endpoint6a", {
  ...
  plop.setGenerator("projection6a", {
...
module.exports = { registerGolden6aGenerators };
